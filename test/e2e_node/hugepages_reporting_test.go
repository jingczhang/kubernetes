...
var _ = SIGDescribe("HugepageAwareMemoryReporting", framework.WithSlow(), framework.WithSerial(), framework.WithDisruptive(), feature.Eviction, func() {
	...
	ginkgo.Context("with feature gate enabled", func() {
		...
		ginkgo.It("should subtract hugepage capacity from memory.available", func(ctx context.Context) {
			...
			framework.Logf("enabled: availableBefore=%d availableAfter=%d hugepages=%d",
				availableBefore, availableAfter, expectedHugepageBytes)

			gomega.Expect(availableBefore-availableAfter).To(
